...
import Crypto.PubKey.ECDSA (verifyDigest, signatureFromIntegers)
...
getPubKeyP256 :: ByteString -> Either String (ECDSA.PublicKey Curve_P256R1)
getPubKeyP256 = cryptoFailableEither . P256.pointFromBinary
...
salesforceZsk :: ECDSA.PublicKey Curve_P256R1
Right salesforceZsk = getPubKeyP256 =<< bytesFromB64 b64SalesforceZsk
...
verifyHelpsfCNAME :: CryptoFailable Bool
verifyHelpsfCNAME = do
  sig <- getSig
  return $ verifyDigest (Proxy :: Proxy Curve_P256R1) salesforceZsk sig (sha256sum sigArgHelpsfCNAME)
  where
    (rb, sb) = BS.splitAt 32 sigHelpsfCNAME
    getSig = signatureFromIntegers Proxy (os2ip rb, os2ip sb)
